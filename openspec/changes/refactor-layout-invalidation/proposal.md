...
This change introduces semantic layout invalidation entry points so callers describe what changed, while `Chart` remains the single owner of the low-level viewport refresh pipeline.
...
- Add internal layout invalidation flags, pending mask merging, and a shared flush pipeline that translates refresh reasons into layout stages.
- Keep the existing synchronous behavior and remove the legacy `adjustPaneViewport(...)` boolean adapter.
- Move main-width convergence into the shared invalidation drain so data, axis, pane, and resize refreshes use the same convergence behavior.
