---
name: openspec-prt
description: 将 Parallel（并行实现）、Review（代码审查）、Test（单元测试）串联执行。Use when the user wants to implement, review, and test in one step.
license: MIT
...
**Input**: 可选指定变更名。若省略则自动推断或通过 AskUserQuestion 让用户选择。
