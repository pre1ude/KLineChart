---
name: "source-command-opsx-parallel"
description: "AI 推断任务并行分组，经用户确认后用 worktree 并发执行，合并回主工作区"
---

# source-command-opsx-parallel

Use this skill when the user asks to run the migrated source command `opsx-parallel`.

## Command Template

将 OpenSpec 变更中的待办任务按依赖关系分组，经用户确认后为每个任务创建独立 git worktree，并发启动 subagent 实现，完成后顺序合并回主工作区。

**Input**: `/opsx:parallel [变更名]`
